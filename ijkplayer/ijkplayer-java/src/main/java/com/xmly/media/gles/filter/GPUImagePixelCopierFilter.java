...
public class GPUImagePixelCopierFilter extends GPUImageFilter {
    ...
    protected volatile boolean isMtcnnNcnn = false;
    ...
    protected void downloadImageToMtcnnNcnnFromTexture(int format) {
        if (mPboBuffers == null) {
            Log.e(TAG, "mPboBuffers is null, downloadImageToMtcnnNcnnFromTexture exit");
            return;
        }
        ...
        XMMtcnnNcnn.getInstance().glReadPixels(0, 0, mRowStride / PIXEL_STRIDE, mPboHeight, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE);
        ...
        XMMtcnnNcnn.getInstance().glMapBufferRange(GLES30.GL_PIXEL_PACK_BUFFER, 0, mPboSize, GLES30.GL_MAP_READ_BIT,
    ...
    public int onDrawToTexture(final int textureId, final FloatBuffer cubeBuffer,
        ...
        if (isMtcnnNcnn) {
            downloadImageToMtcnnNcnnFromTexture(FORMAT_RGBA8888);
        } else if (mNativeRecorder != null) {
    ...
    public void isMtcnn(boolean isMtcnn) {
        this.isMtcnnNcnn = isMtcnn;
    }
}
