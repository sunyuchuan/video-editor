...
import com.xmly.media.camera.view.encoder.video.ImageEncoderCore.OnImageEncoderListener;
...
public class TextureMovieEncoder implements Runnable {
    ...
    private XMMediaRecorder mNativeRecoder = null;
    ...
    public TextureMovieEncoder(XMMediaRecorder recorder) {
        mRunOnDraw = new LinkedList<Runnable>();
        mFilter = GPUImageFilterFactory.CreateFilter(mFilterType);
        mNativeRecoder = recorder;
    }

    public void setOnImageEncoderListener(OnImageEncoderListener l) {
    ...
    public static class EncoderConfig {
        ...
        public EncoderConfig(int width, int height, EGLContext sharedEglContext) {
            mWidth = width;
            mHeight = height;
        ...
        public void handleMessage(Message inputMessage) {
            ...
            switch (what) {
                ...
                case MSG_QUIT:
                    ...
                    break;
                default:
                    Log.e(TAG, "Unhandled msg what=" + what);
            }
        }
    }
...
//        mVideoEncoder.drainEncoder(false);
        synchronized (this) {
            runAll(mRunOnDraw);
            if (mFilter != null) {
                mFilter.onDraw(mTextureId, gLCubeBuffer, gLTextureBuffer);
            }
...
//        mVideoEncoder.drainEncoder(true);
    ...
    private void handleSetTexture(int id) {
        if(VERBOSE) Log.i(TAG, "handleSetTexture " + id);
        mTextureId = id;
    }
    ...
    private void prepareEncoder(EGLContext sharedContext, int width, int height) {
        ...
        mVideoEncoder.setNativeRecorder(mNativeRecoder);
        mVideoEncoder.setListener(mIXMCameraRecorderListener);
        mVideoWidth = width;
        mVideoHeight = height;
        try {
    ...
    public void setFilter(final XMFilterType filtertype, final int width, final int height) {
    ...
    public void setNativeRecorder(XMMediaRecorder recorder) {
        mNativeRecoder = recorder;
        if(mVideoEncoder != null) {
            mVideoEncoder.setNativeRecorder(recorder);
        }
    }
