...
public class XMCameraView extends XMBaseView implements ICameraCallback {
    ...
    public void setMtcnnNcnnModelPath(String path) {
        XMMtcnnNcnn.getInstance().model_init(path);
    }

    public void startRecorder(String outputPath) throws IllegalStateException {
        synchronized (this) {
            if (getStatus()) {
                Log.d(TAG, "start : this is runing, pls waiting stop");
    ...
    public void startCameraPreview() {
        if(mRenderer != null)
            ((XMCameraRenderer) mRenderer).cleanCameraRunOnDraw();
    ...
    private void stopMtcnnNcnn() {
        XMMtcnnNcnn.getInstance().stop();
    }

    private void releaseMtcnnNcnn() {
        XMMtcnnNcnn.getInstance().release();
    }
}
