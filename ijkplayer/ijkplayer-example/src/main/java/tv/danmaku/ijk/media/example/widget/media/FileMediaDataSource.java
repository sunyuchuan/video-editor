...
public class FileMediaDataSource implements IMediaDataSource {
    ...
    public FileMediaDataSource(String path) {
        Log.d(TAG, "FileMediaDataSource");
        mPath = path;
    }
    ...
    public void open(String path, String header) throws IOException {
        try {
            ...
            Log.d(TAG, "oop no such file!");
        }
    }

    @Override
    public int readAt(long position, byte[] buffer, int size) throws IOException {
        //Log.d(TAG, "file read from java " +  " position " + position + " size " + size);
